...
EnumObj<SYSTEM_HANDLE_TABLE_ENTRY_INFO>* NTapi::GetHandlesProcess(DWORD Pid)
{
    ...
    NtQuerySystemInformation(SystemHandleInformation, ppi,sizeof(SYSTEM_HANDLE_INFORMATION), &SysBufLength);
    
    ULONG nOh = ppi->NumberOfHandles;
    delete ppi;
    ppi = (SYSTEM_HANDLE_INFORMATION*)std::calloc(1, SysBufLength);

    NtQuerySystemInformation(SystemHandleInformation, ppi, SysBufLength, nullptr);
...
void WINAPI NTapi::__enum_process__(std::function<bool(PSYSTEM_PROCESS_INFORMATION)> callback) 
{
    ...
    NtQuerySystemInformation(SystemProcessInformation, temp_process.Entry, temp_process.size, nullptr);
...
[[nodiscard]] MODULE_ENTRY * WINAPI NTapi::GetModules()
{
    ...
    MODULE_ENTRY * ModuleEntry = nullptr;
    do
    {
        PLDR_DATA_TABLE_ENTRY pdte = ((PLDR_DATA_TABLE_ENTRY)(current - 1));
        std::wstring n(pdte->BaseDllName.Buffer);

        ModuleEntry = new MODULE_ENTRY{std::wstring(pdte->BaseDllName.Buffer),std::wstring(pdte->FullDllName.Buffer),(PVOID)pdte->DllBase,ModuleEntry};
...
MODULE_ENTRY * WINAPI NTapi::GetImportModules(DEFAULT_SIZE ModuleAddress)
{
    MODULE_ENTRY* ModuleEntry = new MODULE_ENTRY{L"",L"",0,nullptr};
    ...
    while (ped->Name != NULL)
    {
        ...
        ModuleEntry = new MODULE_ENTRY{ std::wstring(szName.begin(),szName.end()),{}, (PVOID)NTapi::GetModuleAddress(szName) ,ModuleEntry };
...
PVOID WINAPI NTapi::ModuleExistEx(MODULE_ENTRY* Modules, std::string  ModulePath)
{
    std::wstring ws(ModulePath.begin(), ModulePath.end());

    do
    {
        if(!wcscmp(Modules->path.c_str(), ws.c_str()))
...
[[nodiscard]] MODULE_ENTRY *  WINAPI NTapi::__get_modules_ex__(DWORD pid)
{
    ...
    MODULE_ENTRY * ModuleEntry = new MODULE_ENTRY;
    ModuleEntry->Next = nullptr;
    HANDLE hauth = NTapi::OpenProcess(pid,PROCESS_QUERY_INFORMATION  | PROCESS_VM_READ);
    ...
    do
    {
        ...
        ModuleEntry = new MODULE_ENTRY{ ReadUTF16EX(hauth, pdte.BaseDllName.Buffer),ReadUTF16EX(hauth, pdte.FullDllName.Buffer),(PVOID)pdte.DllBase,ModuleEntry};
...
[[nodiscard]] MODULE_ENTRY * WINAPI NTapi::GetModulesEx(DWORD pid)
{
...
bool NTapi::FileExistA(const char* PathFile)
{
    return std::ifstream(PathFile).is_open();
}

bool NTapi::FileExistW(const wchar_t * PathFile)
{
    return std::ifstream(PathFile).is_open();
}
